...
class computeRNN(nn.Module):
    ...
    def forward(self,input,_state):
        ...
        # hidden_tmp = Variable(torch.zeros(T, batch, self.hidden_size))  # hidden ->[batch,hidden_size]

        if _state is None:
            _state = Variable(torch.zeros(T, batch, self.hidden_size))  # hidden ->[batch,hidden_size]
        ...
        for t in range(T):
            ...
            hidden[t] = self.tanh(a[t])
            o[t] = self.hidden2out(hidden[t])  # [batch,hidden_size]*[hidden_size,n_class]->[batch,n_class]
...
input_size=2       #一个序列的长度
n_hidden = 12      #隐层神经元数目
target_size = 1     #输出的尺寸
rnn = computeRNN(in_feature=input_size,hidden_size=n_hidden,n_class=target_size)

#
# dummy_input = Variable(torch.randn(99,3,2))   #[torch.FloatTensor of size Nxinput_size],成员都是0
# hidden=None
# output,hidden = rnn(dummy_input,hidden)        #得到[seq_num*target_size],[1*128]
#
# with SummaryWriter(comment='RNN') as w:
#     w.add_graph(rnn, (dummy_input,hidden,))
...
# dummy_input = Variable(torch.randn(99,3,2))   #[torch.FloatTensor of size Nxinput_size],成员都是0
# dummy_hidden=None
# output,dummy_hidden = rnn(dummy_input,dummy_hidden)        #得到[seq_num*target_size],[1*128]
#
# with SummaryWriter(comment='RNN') as w:
#     w.add_graph(rnn, (dummy_input,dummy_hidden,))