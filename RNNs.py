...
class computeRNN(nn.Module):
    ...
    def forward(self,input,pre_state):
        T=input.shape[0]
        batch=input.shape[1]
        a=Variable(torch.zeros(T,self.hidden_size))             #a-> [T,hidden_size]
        o=Variable(torch.zeros(T,self.n_class))                 #o ->[T,n_class]
        ...
        if pre_state is None:
            ...
            hidden = Variable(torch.zeros(batch, self.hidden_size))  # hidden ->[batch,hidden_size]
        for t in range(T):
...
dummy_input = Variable(torch.randn(99,3,2))   #[torch.FloatTensor of size Nxinput_size],成员都是0
dummy_hidden=None
output,hidden = rnn(dummy_input,dummy_hidden)        #得到[seq_num*target_size],[1*128]
